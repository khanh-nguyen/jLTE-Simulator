...
public class MaxCISector extends AbstractSector {

    public static MaxCISector fromXML(Configuration config, Node xml, ENodeB eNodeB, Location location) {
        SectorParams params = new SectorParams(xml);
        return new MaxCISector(config, params.getSectorId(), eNodeB, location, params.getTxPower(),
                params.getAzimuth(), params.getHeight(), params.getDowntilt(), params.getAntennaGain());
    }

    private MaxCISector(Configuration config, int sectorId, final ENodeB eNodeB, final Location loc, double txPower,
    ...
    protected void allocateRB(final int iteration, final int subframe) {
        ...
        for (ResourceBlock RB : unscheduledRBs) {
            ...
            Collections.sort(toSchedule, UEComparators.getRBSignalComparator(RB.id, isDL));
